...
PREPS:
- round values up
- convert to string and then add (calcSticky)
- return the action based on the operation
- ronud final value again
*/

function stickyCalc(operation, val1, val2) {
	const toString = (value) => Math.round(value).toString();
	const calcSticky = (num1, num2) => Number(toString(num1) + toString(num2));
	const action = {
		"-": calcSticky(val1, val2) - val2,
		"+": calcSticky(val1, val2) + val2,
		"/": calcSticky(val1, val2) / val2,
		"*": calcSticky(val1, val2) * val2,
	};
	return Math.round(action[operation]);
}
