...
- round up total simple
- calcualte compound interest
- round up total compound 
*/

function interest(p, r, n) {
	const simpleInterest = p * (1 + r * n);
	const totalSimple = Math.round(simpleInterest);
	const compoundInterest = p * Math.pow(1 + r, n);
	const totalCompond = Math.round(compoundInterest);
	return [totalSimple, totalCompond];
}
